/*Author: cartobaldrica*/
/*Chapter 6 Example 3.7 code for MegaCities.geojson*/
...
function calcStats(data, attributes) {
  ...
  var allValuesFromAttributes = [];
  for (var city of data.features) {
    for (var column of attributes) {
      var valueFromAttributes = city.properties[column];
      allValuesFromAttributes.push(valueFromAttributes);
    }
  }
 
  //allValues array is the same as allValuesFromAttributes array
...
function pointToLayerMegaCities(feature, latlng, attributes) {
...
function createPropSymbols(data, attributes) {
  ...
  L.geoJson(data, {
    pointToLayer: function (feature, latlng) {                          //this pointToLayer is a property of L.geoJson
      return pointToLayerMegaCities(feature, latlng, attributes);       //this pointToLayer is the custom function defined above
    },
  }).addTo(map);
}
...
function createLegend(attributes) {
  ...
  updateLegend(attributes[0]); //change the legend-as-created from DataStats values to max-mean-min of first city
}
